...
from bartdataset_normal_abs import get_dist_loader, get_loader
            ...
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch_idx, batch in enumerate(self.train_loader, start=1):
                    ...
                    nll,enc_loss,dec_loss,_,_= self.model(input_ids= encoder_input_ids,
                                                            attention_mask= shap_attention_mask,
                                                            decoder_input_ids=decoder_input_ids,
                                                            decoder_attention_mask=decoder_attention_mask,
                                                            shap_attention_mask= shap_attention_mask,
                                                            overall = overall,                                                    
                                                            labels = label,random_mask=False)
                    ...
                    running_avg_enc_loss = cal_running_avg_loss(enc_loss.item(), running_avg_loss)
                    running_avg_dec_loss = cal_running_avg_loss(dec_loss.item(), running_avg_loss)
