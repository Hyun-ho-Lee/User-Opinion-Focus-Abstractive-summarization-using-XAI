        ...
        with torch.autograd.set_detect_anomaly(True):
            for epoch in range(1, self.args.num_epochs+1):
                ...
                for batch_idx, batch in enumerate(self.train_loader, start=1):
                    ...
                    running_avg_enc_loss = cal_running_avg_loss(0.1*enc_loss.item(), running_avg_enc_loss)
                    running_avg_dec_loss = cal_running_avg_loss(0.1*dec_loss.item(), running_avg_dec_loss)
                    running_avg_summary_loss = cal_running_avg_loss(0.8*nll.item(), running_avg_loss)
